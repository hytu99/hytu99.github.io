...
点$\mathbf{\mathbf{x}}_i$到线$<w, \mathbf{\mathbf{x}}> + b = 0$的距离$d(\mathbf{\mathbf{x}}_i;\mathbf{w},b) = \dfrac{y_i(\langle \mathbf{w}, \mathbf{\mathbf{x}}_i \rangle + b)}{\Vert \mathbf{w} \Vert_2}$.
...
**Proof:** $\forall (\lambda, \mu), q(\lambda, \mu) = \inf \limits _ {\mathbf{x} \in X} L(\mathbf{x}, \lambda, \mu) \le \inf \limits _ {\mathbf{x} \in D} L(\mathbf{x}, \lambda, \mu) = f ^ *$
...
$$
\max q(\alpha, u) = - \frac{1}{2} \sum _ {i =1} ^ n \sum _ {j = 1} ^ n \alpha_i \alpha_j y_i y _ j <{\bf \mathbf{x}} _ i, {\bf \mathbf{x}} _ j> + \sum _ {i = 1} ^ n \alpha _ i \\\\
